...
description: Strip word-level and sentence-level AI tells from a draft, using the user's anti-ai-writing rulebook as the authority. Use when the user says "humanize this", "strip the AI tells", "run the word pass", "de-slop this", or when polish-and-score reaches its word-level humanizer step. Pairs with stay-human, which fixes narrative structure — this one fixes the words, phrases, punctuation, and rhythm. Never rewrites the argument, only how it reads.
---
...
The authority for every rule here is the user's rulebook, reached through `voice.style_guide_path` (`anti-ai-writing.md`). It is a voice file, not content storage, so it is read directly and never routes through the storage contract. This skill is the operational form of it: same rules, run as a repeatable checklist instead of depending on the model reading 2,700 words carefully every time.
...
Delete or replace every entry in the rulebook's banned-word table. The recurring offenders: leverage (verb), robust, seamless, comprehensive, streamline, unlock, harness, journey, crucial, impactful, ensure, actionable, ecosystem, deep dive, unpack, move the needle. Replace with the plain word the table gives, or rewrite the sentence.
...
- **"Not just X — it's Y."** State the thing flat. The voice profile bans this construction independently.
...
- **Dramatic fragments for false tension.** Fragments are in voice as punch lines ("The process did."), not as manufactured suspense. Keep the former, cut the latter.
...
- **Em dashes: zero.** A standing rule in `voice.sources`, deliberately tighter than the rulebook's one-per-500-words allowance. Zero is the target and it overrides the looser limit. Convert to commas, colons, or periods. Watch for the parenthetical pair, the dramatic reveal, the list intro, and stacked dashes in one sentence.
...
Cut "So,", "Well,", "Now,", "Look,", "Basically,", "Essentially,", "Certainly,", "Interestingly,". Note the exception: the voice opens sentences with And, But, and Then on purpose. Leave those alone.

### 6. Rhythm and tone

- **Break the metronome.** A run of 15-to-20-word sentences reads generated. The default is short declaratives with an occasional long list-loaded sentence. Vary paragraph length too — one-sentence paragraphs are in voice.
...
3. Would the user say this out loud to a colleague? If not, rewrite.
4. Does this add information or just sound impressive? If the latter, cut it.

## Hard rules

- **One spelling convention throughout, the user's.** It is stated in `voice.sources`. If an earlier pass in the chain left the other convention behind — optimised, organisation, realise, behaviour, whilst, when the convention is American English — fix them here. This is a known recurring drift from `stay-human`, which specifies British English in its own rules.
...
- **Never invent a number, a client, or an anecdote.** If a sentence needs a specific and there isn't one, flag it for the user instead of filling the gap.
- **Keep the voice's deliberate breaks.** Sentence fragments, sentences opening with And or But, and one-sentence paragraphs are the voice's, not AI tells. Do not normalize them out.
...
- **No client names.** An engagement listed in `firewall.walled_engagements` never appears in a draft or anywhere in the content system, on either backend.
