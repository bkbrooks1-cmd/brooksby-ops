...
**Check-in, in chat.** The probes run at the top of phase 1, before capture and before any source is read. Failures print as the first lines of the run, before meetings, in the same three-fact form, and each one also lands in the tracker task below. Healthy connectors get no line. This replaces the older "open with 'Gmail unavailable' and continue" rule with a probe that runs whether or not the source was reached during the check-in itself — the old rule only caught a source that threw while being read, which is exactly the failure a silently-empty connector does not produce.

## The tracker task

A failure that only ever prints is a failure the user can scroll past, which is
how one stayed invisible for six weeks. `daily-checkin` therefore turns every
probe result into a row in Tasks, at step 1b. `render-daybook` does not — it is a
read layer and writes nothing but its own artifact id.

**The task is keyed by name and the name never changes:** `Connector watch:
<connector>`. Everything volatile — silence age, last good signal, the named fix
— lives in the body and is refreshed on each pass. A name carrying a date cannot
be deduped against, and three weeks of one dead connector becomes twenty tasks.

The task is also the **state**. The check-in keeps no memory between runs, so
"has this failed before?" is answered by whether the task is open. That is why it
is written on the first failure, not the second.

| Probe | Open task? | Action | Priority |
|---|---|---|---|
| fails | no | create it, say **"strike one"** | **P3** |
| fails | yes | escalate, roll the due date, refresh the three facts, **hard stop** | **P1** |
| passes | yes | mark **Done**, append a recovery note | — |
| passes | no | nothing at all — no task, no line, no banner | — |

Every tracker task carries Type = Admin, Source = Planning, Due date = today, and
the Business Admin internal bucket as its Engagement. A dead connector is
internal ops; it never routes to a client row and never carries a null
engagement.

Closing the task on recovery is what resets the count. A recovered connector
whose task stays open makes the next genuine first failure read as a second one.
